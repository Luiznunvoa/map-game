        ...
        <div class="flex justify-between items-center mb-6">
          ...
          <div class="flex gap-2">
            ...
            <Button onClick={fetchRooms} disabled={isRefreshing()}>
              <Show when={!isRefreshing()} fallback="Refreshing...">Refresh</Show>
            </Button>
            <Button class="bg-red-600 hover:bg-red-700" onClick={() => logoutMutate()}>Sair</Button>
          </div>
        </div>
        
        <RoomList rooms={rooms()} isLoading={isLoading()} onJoinRoom={(id) => navigate(`/room/${id}`)} />
      </div>

      <CreateRoomModal 
        isOpen={showCreateModal()} 
        isLoading={createRoomResource.loading}
        errorMessage={createRoomResource.error?.message || (createRoomResource.error ? 'Error' : undefined)}
