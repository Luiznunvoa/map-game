...
export function setupScene(ctx: IGameEngine, onFrame: (state: FrameState) => void): void {
...
export function setupMapEntity(ctx: IGameEngine['map']): void {
  ctx.map?.dispose()
...
export function handleFrame(ctx: IGameEngine, state: FrameState): void {
